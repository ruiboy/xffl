    ...
    >
      <div class="relative z-10 w-full max-w-lg mx-4 rounded-xl border border-border bg-surface-raised p-6 shadow-2xl">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h3 class="text-base font-semibold text-text">Link Squad Member</h3>
...
import { ADD_AFL_PLAYER, ADD_AFL_PLAYER_SEASON } from '../api/mutations'

// Resolves a squad member to an AFL player_season for the assigned FFL season's
// AFL season. Unlike FflPlayerLinkModal it does not touch FFL — the squad
// importer creates the player_season — so it emits the AFL player_season handle.
...
const emit = defineEmits<{
  close: []
  linked: [{ aflPlayerSeasonId: string; resolvedName: string; resolvedClub: string }]
}>()
...
// Resolves the afl.player_season for the assigned FFL season's AFL season. If the
// player's latest known player_season is for a different AFL season, create the
// player_season for this season at their most recently known club.
async function resolveAflPlayerSeasonId(p: any): Promise<string> {
  ...
  if (!currentClubSeason) {
    throw new Error(
      latest
        ? `Could not find ${clubName} in this AFL season to link this player.`
  ...
  if (!id) throw new Error('Failed to create AFL player season for this season')
  return id
}
