...
func (r *RoundRepository) FindByAFLRoundID(ctx context.Context, aflRoundID int) (domain.Round, error) {
	row, err := r.q.FindRoundByAFLRoundID(ctx, int32(aflRoundID))
	if errors.Is(err, pgx.ErrNoRows) {
...
func (r *MatchRepository) hydrateClubMatch(ctx context.Context, cm *domain.ClubMatch) error {
	...
	for i, pmRow := range pmRows {
		cm.PlayerMatches[i] = toPlayerMatch(pmRow.ID, pmRow.ClubMatchID, pmRow.PlayerSeasonID,
			pmRow.Position, pmRow.Status, pmRow.DrvAflStatus, pmRow.BackupPositions, pmRow.InterchangePosition, pmRow.DisplayOrder, pmRow.Notes, pmRow.DrvScore, pmRow.AflPlayerMatchID)
	}
	return nil
}
...
func (r *ClubMatchRepository) FindFinalSuperbyesBySeasonID(ctx context.Context, seasonID int) ([]domain.SuperbyeClubMatch, error) {
	rows, err := r.q.FindFinalFflSuperbyesBySeasonID(ctx, int32(seasonID))
	if err != nil {
		return nil, err
	}
	out := make([]domain.SuperbyeClubMatch, len(rows))
	for i, row := range rows {
		out[i] = domain.SuperbyeClubMatch{
			MatchID:      int(row.MatchID),
...
func (r *PlayerRepository) FindByAFLPlayerID(ctx context.Context, aflPlayerID int) (domain.Player, error) {
	row, err := r.q.FindPlayerByAFLPlayerID(ctx, int32(aflPlayerID))
	if err != nil {
		return domain.Player{}, err
	}
	return playerFromRow(row.ID, row.AflPlayerID), nil
...
func toPlayerMatch(id, clubMatchID, playerSeasonID int32, position, status, drvAflStatus *string, backupPositions, interchangePosition *string, displayOrder int32, notes *string, score *int32, aflPlayerMatchID *int32) domain.PlayerMatch {
...
func (r *PlayerMatchRepository) FindByClubMatchID(ctx context.Context, clubMatchID int) ([]domain.PlayerMatch, error) {
	...
	for i, row := range rows {
		out[i] = toPlayerMatch(row.ID, row.ClubMatchID, row.PlayerSeasonID,
			row.Position, row.Status, row.DrvAflStatus, row.BackupPositions, row.InterchangePosition, row.DisplayOrder, row.Notes, row.DrvScore, row.AflPlayerMatchID)
	}
	return out, nil
}
...
func (r *PlayerMatchRepository) FindByID(ctx context.Context, id int) (domain.PlayerMatch, error) {
	...
	return toPlayerMatch(row.ID, row.ClubMatchID, row.PlayerSeasonID,
		row.Position, row.Status, row.DrvAflStatus, row.BackupPositions, row.InterchangePosition, row.DisplayOrder, row.Notes, row.DrvScore, row.AflPlayerMatchID), nil
}
...
func toPlayerSeason(id, playerID, clubSeasonID, aflPlayerSeasonID int32, fromRoundID, toRoundID *int32, notes *string, costCents *int32) domain.PlayerSeason {
	return domain.PlayerSeason{
		ID:                int(id),
		PlayerID:          int(playerID),
		ClubSeasonID:      int(clubSeasonID),
		AFLPlayerSeasonID: int(aflPlayerSeasonID),
		FromRoundID:       int32PtrToIntPtr(fromRoundID),
		ToRoundID:         int32PtrToIntPtr(toRoundID),
		Notes:             notes,
		CostCents:         int32PtrToIntPtr(costCents),
	}
}
...
func (r *PlayerSeasonRepository) FindByAFLPlayerSeasonID(ctx context.Context, aflPlayerSeasonID int) ([]domain.PlayerSeason, error) {
	rows, err := r.q.FindPlayerSeasonsByAFLPlayerSeasonID(ctx, int32(aflPlayerSeasonID))
	if err != nil {
		return nil, err
	}
	out := make([]domain.PlayerSeason, len(rows))
...
func (r *PlayerSeasonRepository) Create(ctx context.Context, playerID int, clubSeasonID int, fromRoundID *int, aflPlayerSeasonID int, costCents *int) (domain.PlayerSeason, error) {
	row, err := r.q.CreatePlayerSeason(ctx, sqlcgen.CreatePlayerSeasonParams{
		PlayerID:          int32(playerID),
		ClubSeasonID:      int32(clubSeasonID),
		FromRoundID:       intPtrToInt32Ptr(fromRoundID),
		AflPlayerSeasonID: int32(aflPlayerSeasonID),
		CostCents:         intPtrToInt32Ptr(costCents),
	})
	if err != nil {
		return domain.PlayerSeason{}, err
	}
...
func (r *PlayerSeasonRepository) UpdateDetails(ctx context.Context, id int, notes *string) (domain.PlayerSeason, error) {
	row, err := r.q.UpdatePlayerSeason(ctx, sqlcgen.UpdatePlayerSeasonParams{
		ID:    int32(id),
		Notes: notes,
	})
	if err != nil {
