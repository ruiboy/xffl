...
def cmd_sql(review_path: str):
    ...
    bad = [r for r in rows if not r['afl_player_id'] or r['confidence'] == 'NO_MATCH']
    if bad:
    ...
    # Group by ffl_club to emit one block per club
    ...
    for ffl_club, players in clubs_seen.items():
        ...
        # ffl.player inserts (one per player; idempotent via NOT EXISTS)
        for p in players:
            pid = int(p['afl_player_id'])
            name = p['matched_name'].replace("'", "''")
            print(f"INSERT INTO ffl.player (afl_player_id, drv_name)")
            print(f"SELECT {pid}, '{name}'")
            print(f"WHERE NOT EXISTS (SELECT 1 FROM ffl.player WHERE afl_player_id = {pid});")

        print()

        # ffl.player_season inserts
        print(f"INSERT INTO ffl.player_season (player_id, club_season_id, from_round_id, afl_player_season_id)")
        print(f"SELECT")
        print(f"    fp.id,")
        print(f"    (SELECT cs.id FROM ffl.club_season cs")
        print(f"     JOIN ffl.club c ON cs.club_id = c.id")
        print(f"     JOIN ffl.season s ON cs.season_id = s.id")
        print(f"     WHERE c.name = '{ffl_club}' AND s.name = 'FFL 2026'),")
        print(f"    (SELECT r.id FROM ffl.round r")
        print(f"     JOIN ffl.season s ON r.season_id = s.id")
        print(f"     WHERE r.name = 'Round 1' AND s.name = 'FFL 2026'),")
        print(f"    (SELECT aps.id FROM afl.player_season aps")
        print(f"     JOIN afl.club_season acs ON aps.club_season_id = acs.id")
        print(f"     JOIN afl.season asn ON acs.season_id = asn.id")
        print(f"     WHERE aps.player_id = fp.afl_player_id AND asn.name = 'AFL 2026')")
        print(f"FROM (VALUES")
        ...
        print(f") AS v(afl_player_id)")
        print(f"JOIN ffl.player fp ON fp.afl_player_id = v.afl_player_id")
